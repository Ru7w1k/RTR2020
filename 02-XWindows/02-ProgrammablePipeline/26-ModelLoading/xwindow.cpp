...
void CreateWindow(void)
{
	...
	XStoreName(gpDisplay, gWindow, "OpenGL | BnW Cube");
...
void initialize(void)
{
	...
	glBufferData(GL_ARRAY_BUFFER, gFaceTri.size()*3*3*sizeof(float), data, GL_STATIC_DRAW);
...
void display(void)
{
	...
	glDrawArrays(GL_TRIANGLES, 0, gFaceTri.size()*3);
