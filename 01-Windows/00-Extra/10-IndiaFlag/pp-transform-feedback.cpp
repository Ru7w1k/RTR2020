...
int initialize(void)
{
	...
	const GLchar* vertexShaderSourceCode = (GLchar*)
		...
		"	s = clamp(s, vec3(-25.0), vec3(25.0));" \
			...
			// interact with the environment! (fake)
		"	float w = velocity.w;" \
		"	if (vec3(p+s).y <= -4.0 && abs(vec3(p+s).x) < 10.5 && abs(vec3(p+s).z) < 10.5)" \
		"	{	" \
		"		s = vec3(0.0);" \
		"		v = vec3(0.0);" \
		"	}	" \
