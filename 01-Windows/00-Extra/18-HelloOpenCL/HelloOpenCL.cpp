...
int main(void) {
  ...
  if (gold == NULL) {
    printf("CPU Memory Fatal Error = Can Not Allocate Enough Memory For Gold "
           "Output Array.\nExitting ...\n");
    cleanup();
    exit(EXIT_FAILURE);
  }
  ...
  if (ret_ocl != CL_SUCCESS) {
    printf(
        "OpenCL Error - clCreateCommandQueue() Failed : %d. Exitting Now ...\n",
        ret_ocl);
    cleanup();
    exit(EXIT_FAILURE);
  }
  ...
  if (ret_ocl != CL_SUCCESS) {
    printf("OpenCL Error - clCreateProgramWithSource() Failed : %d. Exitting "
           "Now ...\n",
           ret_ocl);
    cleanup();
    exit(0);
  ...
  deviceInput1 =
      clCreateBuffer(oclContext, CL_MEM_READ_ONLY, size, NULL, &ret_ocl);
  if (ret_ocl != CL_SUCCESS) {
    printf("OpenCL Error - clCreateBuffer() Failed For 1st Input Array : %d. "
           "Exitting Now ...\n",
           ret_ocl);
    cleanup();
    exit(EXIT_FAILURE);
  ...
  deviceInput2 =
      clCreateBuffer(oclContext, CL_MEM_READ_ONLY, size, NULL, &ret_ocl);
  if (ret_ocl != CL_SUCCESS) {
    printf("OpenCL Error - clCreateBuffer() Failed For 2nd Input Array : %d. "
           "Exitting Now ...\n",
           ret_ocl);
    cleanup();
    exit(EXIT_FAILURE);
  ...
  deviceOutput =
      clCreateBuffer(oclContext, CL_MEM_WRITE_ONLY, size, NULL, &ret_ocl);
  if (ret_ocl != CL_SUCCESS) {
    printf("OpenCL Error - clCreateBuffer() Failed For 2nd Input Array : %d. "
           "Exitting Now ...\n",
           ret_ocl);
  ...
  ret_ocl = clSetKernelArg(
      oclKernel, 0, sizeof(cl_mem),
      (void *)&deviceInput1); // 'deviceInput1' maps to 'in1' param of kernel
                              // function in .cl file
  if (ret_ocl != CL_SUCCESS) {
  ...
  ret_ocl = clSetKernelArg(
      oclKernel, 1, sizeof(cl_mem),
      (void *)&deviceInput2); // 'deviceInput2' maps to 'in2' param of kernel
                              // function in .cl file
  if (ret_ocl != CL_SUCCESS) {
  ...
  ret_ocl = clSetKernelArg(
      oclKernel, 2, sizeof(cl_mem),
      (void *)&deviceOutput); // 'deviceOutput' maps to 'out' param of kernel
                              // function in .cl file
  if (ret_ocl != CL_SUCCESS) {
  ...
  ret_ocl = clSetKernelArg(
      oclKernel, 3, sizeof(cl_int),
      (void
           *)&iNumberOfArrayElements); // 'iNumberOfArrayElements' maps to 'len'
                                       // param of kernel function in .cl file
  if (ret_ocl != CL_SUCCESS) {
    printf("OpenCL Error - clSetKernelArg() Failed For 4th Argument : %d. "
           "Exitting Now ...\n",
           ret_ocl);
    cleanup();
    exit(EXIT_FAILURE);
  ...
  ret_ocl = clEnqueueWriteBuffer(oclCommandQueue, deviceInput1, CL_FALSE, 0,
                                 size, hostInput1, 0, NULL, NULL);
  if (ret_ocl != CL_SUCCESS) {
    printf("OpenCL Error - clEnqueueWriteBuffer() Failed For 1st Input Device "
           "Buffer : %d. Exitting Now ...\n",
           ret_ocl);
    cleanup();
    exit(EXIT_FAILURE);
  }

  ret_ocl = clEnqueueWriteBuffer(oclCommandQueue, deviceInput2, CL_FALSE, 0,
                                 size, hostInput2, 0, NULL, NULL);
  if (ret_ocl != CL_SUCCESS) {
    printf("OpenCL Error - clEnqueueWriteBuffer() Failed For 2nd Input Device "
           "Buffer : %d. Exitting Now ...\n",
           ret_ocl);
    cleanup();
    exit(EXIT_FAILURE);
  ...
  globalWorkSize = roundGlobalSizeToNearestMultipleOfLocalSize(
      localWorkSize, iNumberOfArrayElements);
  ...
  // read back result from the device (i.e from deviceOutput) into cpu variable
  // (i.e hostOutput)
  ret_ocl = clEnqueueReadBuffer(oclCommandQueue, deviceOutput, CL_TRUE, 0, size,
                                hostOutput, 0, NULL, NULL);
  if (ret_ocl != CL_SUCCESS) {
    printf(
        "OpenCL Error - clEnqueueReadBuffer() Failed : %d. Exitting Now ...\n",
        ret_ocl);
    cleanup();
    exit(EXIT_FAILURE);
  }
  ...
  if (bAccuracy == true)
    sprintf(str, "%s",
            "Comparison Of Output Arrays On CPU And GPU Are Accurate Within "
            "The Limit Of 0.000001");
  else
    sprintf(str, "%s",
            "Not All Comparison Of Output Arrays On CPU And GPU Are Accurate "
            "Within The Limit Of 0.000001");

  printf("1st Array Is From 0th Element %.6f To %dth Element %.6f\n",
         hostInput1[0], iNumberOfArrayElements - 1,
         hostInput1[iNumberOfArrayElements - 1]);
  printf("2nd Array Is From 0th Element %.6f To %dth Element %.6f\n",
         hostInput2[0], iNumberOfArrayElements - 1,
         hostInput2[iNumberOfArrayElements - 1]);
  ...
  printf("3nd Array Is From 0th Element %.6f To %dth Element %.6f\n",
         hostOutput[0], iNumberOfArrayElements - 1,
         hostOutput[iNumberOfArrayElements - 1]);
...
void vecAddHost(const float *pFloatData1, const float *pFloatData2,
                float *pFloatResult, int iNumElements) {
  int i;
