...
HRESULT initialize(void)
{
	...
	const char *vertexShaderSourceCode =
		...
		"};                                                                                   \n" \
		"                                                                                     \n" \
		"vertex_output main(float4 pos: POSITION, float4 normal: NORMAL, float4 color: COLOR, uint idx: SV_InstanceID) \n" \
		"{                                                                                    \n" \
	...
	const char *pixelShaderSourceCode =
		...
		"       float4 diffuse = ld * input.color * tn_dot_ld;                    \n" \
		"       float4 specular = ls * input.color * pow(max(dot(reflection_vector, nviewer_vector), 0.0), material_shininess); \n" \
	...
	ZeroMemory(&bufferDesc_VertexBuffer, sizeof(D3D11_BUFFER_DESC));
	bufferDesc_VertexBuffer.Usage = D3D11_USAGE_DYNAMIC;
	bufferDesc_VertexBuffer.ByteWidth = sizeof(colorData);
	bufferDesc_VertexBuffer.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	bufferDesc_VertexBuffer.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
	...
	// copy colorData into above buffer
	ZeroMemory(&mappedSubresource, sizeof(D3D11_MAPPED_SUBRESOURCE));
	gpID3D11DeviceContext->Map(gpID3D11Buffer_VertexBuffer_instance, 0,
		D3D11_MAP_WRITE_DISCARD, 0, &mappedSubresource);
	memcpy(mappedSubresource.pData, colorData, sizeof(colorData));
...
void display(void)
{
	...
	stride = sizeof(float) * 3;
	offset = 0;
	gpID3D11DeviceContext->IASetVertexBuffers(2, 1, &gpID3D11Buffer_VertexBuffer_instance, &stride, &offset);
	...
	// translation
	translationMatrix = XMMatrixTranslation(-4.0f, 0.0f, 6.0f);
	...
	gpID3D11DeviceContext->DrawIndexedInstanced(gNumElements, 10, 0, 0, 0);
