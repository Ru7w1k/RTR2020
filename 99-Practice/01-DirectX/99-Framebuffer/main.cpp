...
// framebuffer 
ID3D11RenderTargetView *gpID3D11RenderTargetView_framebuffer = NULL;
ID3D11ShaderResourceView *gpID3D11ShaderResourceView_framebuffer = NULL;
ID3D11SamplerState *gpID3D11SamplerState_framebuffer = NULL;
...
HRESULT resize(int width, int height)
{
	...
	if (gpID3D11RenderTargetView_framebuffer)
	{
		gpID3D11RenderTargetView_framebuffer->Release();
		gpID3D11RenderTargetView_framebuffer = NULL;
	}
	...
	// create one more render target view with the same properties
	// fetch the texture desc of back buffer texture
	D3D11_TEXTURE2D_DESC textureDesc;
	pID3D11Texture2D_BackBuffer->GetDesc(&textureDesc);
	...
	// create texture with the above texture desc
	...
	if (FAILED(hr))
	{
		...
		ComErrorDescriptionString(ghwnd, hr);
		return(hr);
	}
	else
	{
		Log("ID3D11Device::CreateRenderTargetView() succeeded for framebuffer..\n");
	}

	// create shader resource view for framebuffer texture
	...
	hr = gpID3D11Device->CreateShaderResourceView(pFramebufferTexture, &SRVDesc, &gpID3D11ShaderResourceView_framebuffer);
	if (FAILED(hr))
	{
		Log("ID3D11Device::CreateShaderResourveView() failed for framebuffer..\n");
		return(hr);
	}
	else
	{
		Log("ID3D11Device::CreateShaderResourveView() succeeded for framebuffer..\n");
	}

	pFramebufferTexture->Release();
	pFramebufferTexture = NULL;
...
void display(void)
{
	...
	gpID3D11DeviceContext->ClearRenderTargetView(gpID3D11RenderTargetView_framebuffer, gClearColor1);
	gpID3D11DeviceContext->ClearDepthStencilView(gpID3D11DepthStencilView, D3D11_CLEAR_DEPTH, 1.0f, 0);

	// set render target as framebuffer
	gpID3D11DeviceContext->OMSetRenderTargets(1, &gpID3D11RenderTargetView_framebuffer, gpID3D11DepthStencilView);
	...
	// bind texture and sampler as pixel shader resource
	gpID3D11DeviceContext->PSSetShaderResources(0, 1, &gpID3D11ShaderResourceView_framebuffer);
	gpID3D11DeviceContext->PSSetSamplers(0, 1, &gpID3D11SamplerState_framebuffer);
...
void ComErrorDescriptionString(HWND hWnd, HRESULT hr)
{
	...
	if (FACILITY_WINDOWS == HRESULT_FACILITY(hr))
		hr = HRESULT_CODE(hr);
	if (FormatMessageA(FORMAT_MESSAGE_ALLOCATE_BUFFER | FORMAT_MESSAGE_FROM_SYSTEM, NULL, hr, MAKELANGID(LANG_NEUTRAL, SUBLANG_DEFAULT), (LPSTR)&szErrorMessage, 0, NULL) != 0)
	{
	...
	MessageBoxA(hWnd, str, "COM Error", MB_OK);
