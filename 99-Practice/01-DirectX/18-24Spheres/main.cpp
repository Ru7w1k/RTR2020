...
HRESULT resize(int width, int height)
{
	...
	char str[255];
	sprintf_s(str, "resized with %d x %d\n", width, height);
	Log(str);
...
void display(void)
{
	...
	if (bLight)
	{
		...
		if (bXLight)
			constantBuffer.LightPosition = XMVectorSet(0.0f, 100.0f*cos(-angle), 100.0f*sin(-angle), 1.0f);

		if (bYLight)
			constantBuffer.LightPosition = XMVectorSet(100.0f*cos(-angle), 0.0f, 100.0f*sin(-angle), 1.0f);

		if (bZLight)
			constantBuffer.LightPosition = XMVectorSet(100.0f*cos(-angle), 100.0f*sin(-angle), 0.0f, 1.0f);
	}
	else
	{
