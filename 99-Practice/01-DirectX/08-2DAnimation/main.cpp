...
HRESULT initialize(void)
{
	...
	// create vertex buffer for position triangle
	...
	bufferDesc_VertexBuffer.ByteWidth = sizeof(float)  *ARRAYSIZE(vertices_triangle);
	bufferDesc_VertexBuffer.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	bufferDesc_VertexBuffer.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;

	hr = gpID3D11Device->CreateBuffer(&bufferDesc_VertexBuffer, NULL, &gpID3D11Buffer_VertexBuffer_Triangle_Position);
	if (FAILED(hr))
	{
	...
	gpID3D11DeviceContext->Map(gpID3D11Buffer_VertexBuffer_Triangle_Position, 0,
	...
	// create vertex buffer for color triangle
	ZeroMemory(&bufferDesc_VertexBuffer, sizeof(D3D11_BUFFER_DESC));
	bufferDesc_VertexBuffer.Usage = D3D11_USAGE_DYNAMIC;
	bufferDesc_VertexBuffer.ByteWidth = sizeof(float)  *ARRAYSIZE(colors_triangle);
	bufferDesc_VertexBuffer.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	bufferDesc_VertexBuffer.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;

	hr = gpID3D11Device->CreateBuffer(&bufferDesc_VertexBuffer, NULL, &gpID3D11Buffer_VertexBuffer_Triangle_Color);
	if (FAILED(hr))
	{
	...
	// copy colors_triangle into above buffer
	ZeroMemory(&mappedSubresource, sizeof(D3D11_MAPPED_SUBRESOURCE));
	gpID3D11DeviceContext->Map(gpID3D11Buffer_VertexBuffer_Triangle_Color, 0,
		D3D11_MAP_WRITE_DISCARD, 0, &mappedSubresource);
	memcpy(mappedSubresource.pData, colors_triangle, sizeof(colors_triangle));
	gpID3D11DeviceContext->Unmap(gpID3D11Buffer_VertexBuffer_Triangle_Color, NULL);

	///////////////////////////////////////////////////////////////////////////////

	// create vertex buffer for position rectangle
	ZeroMemory(&bufferDesc_VertexBuffer, sizeof(D3D11_BUFFER_DESC));
	bufferDesc_VertexBuffer.Usage = D3D11_USAGE_DYNAMIC;
	bufferDesc_VertexBuffer.ByteWidth = sizeof(float)  *ARRAYSIZE(vertices_rectangle);
	bufferDesc_VertexBuffer.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	bufferDesc_VertexBuffer.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;

	hr = gpID3D11Device->CreateBuffer(&bufferDesc_VertexBuffer, NULL, &gpID3D11Buffer_VertexBuffer_Rectangle_Position);
	if (FAILED(hr))
	{
		Log("ID3D11Device::CreateBuffer() failed for vertex buffer position..\n");
		return(hr);
	...
	// copy vertices_rectangle into above buffer
	ZeroMemory(&mappedSubresource, sizeof(D3D11_MAPPED_SUBRESOURCE));
	gpID3D11DeviceContext->Map(gpID3D11Buffer_VertexBuffer_Rectangle_Position, 0,
		D3D11_MAP_WRITE_DISCARD, 0, &mappedSubresource);
	memcpy(mappedSubresource.pData, vertices_rectangle, sizeof(vertices_rectangle));
	gpID3D11DeviceContext->Unmap(gpID3D11Buffer_VertexBuffer_Rectangle_Position, NULL);

	///////////////////////////////////////////////////////////////////////////////

	// create vertex buffer for color rectangle
	ZeroMemory(&bufferDesc_VertexBuffer, sizeof(D3D11_BUFFER_DESC));
	bufferDesc_VertexBuffer.Usage = D3D11_USAGE_DYNAMIC;
	bufferDesc_VertexBuffer.ByteWidth = sizeof(float)  *ARRAYSIZE(colors_rectangle);
	bufferDesc_VertexBuffer.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	bufferDesc_VertexBuffer.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;

	hr = gpID3D11Device->CreateBuffer(&bufferDesc_VertexBuffer, NULL, &gpID3D11Buffer_VertexBuffer_Rectangle_Color);
	if (FAILED(hr))
	{
		Log("ID3D11Device::CreateBuffer() failed for vertex buffer color..\n");
		return(hr);
	...
	// copy colors_rectangle into above buffer
	ZeroMemory(&mappedSubresource, sizeof(D3D11_MAPPED_SUBRESOURCE));
	gpID3D11DeviceContext->Map(gpID3D11Buffer_VertexBuffer_Rectangle_Color, 0,
		D3D11_MAP_WRITE_DISCARD, 0, &mappedSubresource);
	memcpy(mappedSubresource.pData, colors_rectangle, sizeof(colors_rectangle));
...
void display(void)
{
	...
	rotationMatrix = XMMatrixRotationY(XMConvertToRadians(-gAngleTriangle));
	...
	// translation
	translationMatrix = XMMatrixTranslation(1.5f, 0.0f, 5.0f);
	rotationMatrix = XMMatrixRotationX(XMConvertToRadians(-gAngleRectangle));
...
void update(void)
{
	...
	gAngleRectangle += 1.0f;
	if (gAngleRectangle >= 360.0f)
		gAngleRectangle = 0.0f;
}
