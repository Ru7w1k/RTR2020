...
public class GLESView extends GLSurfaceView implements GLSurfaceView.Renderer, OnDoubleTapListener, OnGestureListener {
    ...
    private void draw() {
        ...
        if (bLight) {
            ...
            switch (axis) {
                case 0:
                    GLES32.glUniform4f(lightPositionUniform, 0.0f, 100.0f*(float)Math.cos(angle), 100.0f*(float)Math.sin(angle), 1.0f);
                    break;

                case 1:
                    GLES32.glUniform4f(lightPositionUniform, 100.0f*(float)Math.cos(angle), 0.0f, 100.0f*(float)Math.sin(angle), 1.0f);
                    break;

                case 2:
                    GLES32.glUniform4f(lightPositionUniform, 100.0f*(float)Math.cos(angle), 100.0f*(float)Math.sin(angle), 0.0f, 1.0f);
                    break;
            }
