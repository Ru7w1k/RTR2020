package com.ru7w1k.pyramidcubetexture;
...
public class GLESView extends GLSurfaceView implements GLSurfaceView.Renderer, OnDoubleTapListener, OnGestureListener {
    ...
    private int[] vboPyramidTex = new int[1];

    private int[] vaoCube    = new int[1];
    private int[] vboCubePos = new int[1];
    private int[] vboCubeTex = new int[1];
    ...
    private void initialize(GL10 gl) {
        ...
        floatBuffer.put(pyramidVertices);
        floatBuffer.position(0);

        // pass to OpenGL buffer
        GLES32.glBufferData(GLES32.GL_ARRAY_BUFFER, pyramidVertices.length * 4, floatBuffer, GLES32.GL_STATIC_DRAW);
        ...
        GLES32.glGenBuffers(1, vboPyramidTex, 0);
        GLES32.glBindBuffer(GLES32.GL_ARRAY_BUFFER, vboPyramidTex[0]);

        // create native buffer to pass to OpenGLES
        byteBuffer = ByteBuffer.allocateDirect(pyramidTexcoords.length * 4);
        byteBuffer.order(ByteOrder.nativeOrder());
        floatBuffer = byteBuffer.asFloatBuffer();
        floatBuffer.put(pyramidTexcoords);
        floatBuffer.position(0);

        // pass to OpenGL buffer
        GLES32.glBufferData(GLES32.GL_ARRAY_BUFFER, pyramidTexcoords.length * 4, floatBuffer, GLES32.GL_STATIC_DRAW);
        ...
        GLES32.glGenBuffers(1, vboCubeTex, 0);
        GLES32.glBindBuffer(GLES32.GL_ARRAY_BUFFER, vboCubeTex[0]);

        // create native buffer to pass to OpenGLES
        byteBuffer = ByteBuffer.allocateDirect(cubeTexcoords.length * 4);
        byteBuffer.order(ByteOrder.nativeOrder());
        floatBuffer = byteBuffer.asFloatBuffer();
        floatBuffer.put(cubeTexcoords);
        floatBuffer.position(0);

        // pass to OpenGL buffer
        GLES32.glBufferData(GLES32.GL_ARRAY_BUFFER, cubeTexcoords.length * 4, floatBuffer, GLES32.GL_STATIC_DRAW);
        GLES32.glVertexAttribPointer(GLESMacros.RMC_ATTRIBUTE_TEXCOORD, 2, GLES32.GL_FLOAT, false, 0, 0);
        GLES32.glEnableVertexAttribArray(GLESMacros.RMC_ATTRIBUTE_TEXCOORD);
        GLES32.glBindBuffer(GLES32.GL_ARRAY_BUFFER, 0);
        ...
        stoneTexture = loadGLTexture(R.raw.stone);
        kundaliTexture = loadGLTexture(R.raw.kundali);
    ...
    private void draw() {
        ...
        GLES32.glBindTexture(GLES32.GL_TEXTURE_2D, stoneTexture);
    ...
    private void uninitialize() {
        ...
        if (vboCubeTex[0] != 0) {
            GLES32.glDeleteBuffers(1, vboCubeTex, 0);
            vboCubeTex[0] = 0;
        }
        ...
        if (vboPyramidTex[0] != 0) {
            GLES32.glDeleteBuffers(1, vboPyramidTex, 0);
            vboPyramidTex[0] = 0;
        }
