...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance,
  ...
  // create window
  hwnd = CreateWindowEx(
      WS_EX_APPWINDOW, szAppName, TEXT("D3D11 | Diffuse Light on Cube"),
...
HRESULT initialize(void) {
  ...
  memcpy(mappedSubresource.pData, normals, sizeof(normals));
...
void display(void) {
  ...
  rotationMatrix = XMMatrixRotationX(XMConvertToRadians(-anglePyramid));
  rotationMatrix *= XMMatrixRotationY(XMConvertToRadians(-anglePyramid));
  rotationMatrix *= XMMatrixRotationZ(XMConvertToRadians(-anglePyramid));
