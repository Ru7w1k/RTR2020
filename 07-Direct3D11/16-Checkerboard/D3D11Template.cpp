...
unsigned char CheckData[CHECK_IMG_WIDTH][CHECK_IMG_HEIGHT][4] = { 0 };
...
HRESULT initialize(void)
{
	...
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
		fprintf_s(gpFile, "ID3D11Device::CreateShaderResourceView() failed for Checkerboard.\n");
		fclose(gpFile);
		return(hr);
	}
	else
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
		fprintf_s(gpFile, "ID3D11Device::CreateShaderResourceView() succeeded for Checkerboard.\n");
		fclose(gpFile);
	}
