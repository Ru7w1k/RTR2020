...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow)
{
	...
	hwnd = CreateWindow(szClassName,
		TEXT("D3D11 | Two Lights on Pyramid"),
		WS_OVERLAPPEDWINDOW,
		100,
		100,
		WIN_WIDTH,
...
HRESULT initialize(void)
{
	...
	const char *vertexShaderSourceCode_PerPixel =
		...
		"	float3 light_direction_blue  : NORMAL3;" \
		"};" \
	...
	bufferDesc_VertexBuffer_Position.ByteWidth = sizeof(float) * ARRAYSIZE(vertices_pyramid);
	...
	bufferDesc_VertexBuffer_Normal.ByteWidth = sizeof(float) * ARRAYSIZE(normals_pyramid);
	...
	gpID3D11DeviceContext->Map(gpID3D11Buffer_VertexBuffer_Normal, 0,
		D3D11_MAP_WRITE_DISCARD, 0, &mappedSubresource);
	memcpy(mappedSubresource.pData, normals_pyramid, sizeof(normals_pyramid));
...
void display(void)
{
	...
	if (gbLight == true)
	{
		...
		constantBuffer.La_Blue = XMVectorSet(lights[1].lightAmbient[0], lights[1].lightAmbient[1], lights[1].lightAmbient[2], lights[1].lightAmbient[3]);
		...
		constantBuffer.Light_Position_Blue = XMVectorSet(lights[1].lightPosition[0], lights[1].lightPosition[1], lights[1].lightPosition[2], lights[1].lightPosition[3]);
