...
ID3D11Buffer *gpID3D11BufferConstantBufferHullShader = NULL;
ID3D11Buffer *gpID3D11BufferConstantBufferDomainShader = NULL;
ID3D11Buffer *gpID3D11BufferConstantBufferPixelShader = NULL;
...
struct CBUFFER_HULL_SHADER {
  ...
  // XMVECTOR is float4
  // 0th : no of line strips
  // 1st : no of line segments
  // cbuffer size must be >= 16 bytes and in multiple of 16 bytes
};
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance,
  ...
  // create window
  hwnd = CreateWindowEx(
      WS_EX_APPWINDOW, szAppName, TEXT("D3D11 | Hull and Domain Shader"),
...
HRESULT initialize(void) {
  ...
  const char *hullShaderSourceCode =
    ...
    "  float noOfStrips = hullConstantFunctionParams[0]; \n" \
    "  float noOfSegments = hullConstantFunctionParams[1]; \n" \
  ...
  hr = D3DCompile(
      hullShaderSourceCode,                // shader source code
      lstrlenA(hullShaderSourceCode) + 1,  // lenght of source code
      "HS",                                // shader stage to compile for
      NULL,                                // macro (if required)
      D3D_COMPILE_STANDARD_FILE_INCLUDE,   // include files
      "main",                              // entry-point function name
      "hs_5_0",                            // shader model
      0,  // compiler flags if any for shader | optimization / debug /
      0,  // compiler flags if any for effect | profiling related flags
      &pID3DBlobHullShaderCode,  // compiled shader code if successful
      &pID3DBlobError            // compilation errors if any
  );
  ...
  if (FAILED(hr)) {
    // error in shader source code
    if (pID3DBlobError != NULL) {
      LogD("D3DCompile() failed for Hull Shader: \n%s.\n",
           (char *)pID3DBlobError->GetBufferPointer());
      pID3DBlobError->Release();
      pID3DBlobError = NULL;
      return (hr);
  ...
  LogD("D3DCompile() succeeded for Hull Shader.\n");

  hr = gpID3D11Device->CreateHullShader(
      pID3DBlobHullShaderCode->GetBufferPointer(),  // source code
      pID3DBlobHullShaderCode->GetBufferSize(),     // source code length
      NULL,                // shared variables between multiple shaders
      &gpID3D11HullShader  // Hull shader object!
  );
  if (FAILED(hr)) {
    LogD("CreateHullShader() failed..\n");
    return (hr);
  }
  LogD("CreateHullShader() succeeded..\n");

  gpID3D11DeviceContext->HSSetShader(gpID3D11HullShader,  // Hull shader object
                                     NULL,  // class linkage array (if any)
                                     0      // class linkage array size (if any)
  );

  if (FAILED(hr)) {
    LogD("HSSetShader() failed..\n");
    return (hr);
  }
  LogD("HSSetShader() succeeded..\n");

  // dont release vertex shader code blob yet, but we can release Hull shader
  if (pID3DBlobHullShaderCode) {
  ...
  const char *domainShaderSourceCode =
    ...
    "   float3 p = (p0 * (1.0 - tessCoord.x) * (1.0 - tessCoord.x) * (1.0 - tessCoord.x)) + \n" \
    "              (p1 * 3.0 * tessCoord.x * (1.0 - tessCoord.x) * (1.0 - tessCoord.x)) +  \n" \
    "              (p2 * 3.0 * tessCoord.x * tessCoord.x * (1.0 - tessCoord.x)) + \n" \
  ...
  hr = D3DCompile(
      domainShaderSourceCode,                // shader source code
      lstrlenA(domainShaderSourceCode) + 1,  // lenght of source code
      "DS",                                  // shader stage to compile for
      NULL,                                  // macro (if required)
      D3D_COMPILE_STANDARD_FILE_INCLUDE,     // include files
      "main",                                // entry-point function name
      "ds_5_0",                              // shader model
      0,  // compiler flags if any for shader | optimization / debug /
      0,  // compiler flags if any for effect | profiling related flags
      &pID3DBlobDomainShaderCode,  // compiled shader code if successful
      &pID3DBlobError              // compilation errors if any
  );
  ...
  if (FAILED(hr)) {
    // error in shader source code
    if (pID3DBlobError != NULL) {
      LogD("D3DCompile() failed for DOmain Shader: \n%s.\n",
           (char *)pID3DBlobError->GetBufferPointer());
      pID3DBlobError->Release();
      pID3DBlobError = NULL;
      return (hr);
  ...
  LogD("D3DCompile() succeeded for Domain Shader.\n");

  hr = gpID3D11Device->CreateDomainShader(
      pID3DBlobDomainShaderCode->GetBufferPointer(),  // source code
      pID3DBlobDomainShaderCode->GetBufferSize(),     // source code length
      NULL,                  // shared variables between multiple shaders
      &gpID3D11DomainShader  // Domain shader object!
  );
  if (FAILED(hr)) {
    LogD("CreateDomainShader() failed..\n");
    return (hr);
  }
  LogD("CreateDomainShader() succeeded..\n");
  ...
  // dont release vertex shader code blob yet, but we can release Domain shader
  if (pID3DBlobDomainShaderCode) {
  ...
  bufferDescConstBufferDesc.ByteWidth = sizeof(CBUFFER_HULL_SHADER);
  ...
  hr = gpID3D11Device->CreateBuffer(
      ...
      NULL,                          // subresource data (used in static buffer)
      &gpID3D11BufferConstantBufferHullShader  // constant buffer !
  );
  if (FAILED(hr)) {
  ...
  ZeroMemory(&bufferDescConstBufferDesc, sizeof(D3D11_BUFFER_DESC));
  bufferDescConstBufferDesc.ByteWidth = sizeof(CBUFFER_DOMAIN_SHADER);
  bufferDescConstBufferDesc.BindFlags = D3D11_BIND_CONSTANT_BUFFER;
  bufferDescConstBufferDesc.Usage = D3D11_USAGE_DEFAULT;
  ...
  hr = gpID3D11Device->CreateBuffer(
      &bufferDescConstBufferDesc,    // buffer desc
      NULL,                          // subresource data (used in static buffer)
      &gpID3D11BufferConstantBufferDomainShader  // constant buffer !
  );
  if (FAILED(hr)) {
    LogD("CreateBuffer() failed for constant buffer..\n");
    return (hr);
  ...
  ZeroMemory(&bufferDescConstBufferDesc, sizeof(D3D11_BUFFER_DESC));
  bufferDescConstBufferDesc.ByteWidth = sizeof(CBUFFER_PIXEL_SHADER);
  bufferDescConstBufferDesc.BindFlags = D3D11_BIND_CONSTANT_BUFFER;
  bufferDescConstBufferDesc.Usage = D3D11_USAGE_DEFAULT;
  ...
  hr = gpID3D11Device->CreateBuffer(
      &bufferDescConstBufferDesc,    // buffer desc
      NULL,                          // subresource data (used in static buffer)
      &gpID3D11BufferConstantBufferPixelShader  // constant buffer !
  );
  if (FAILED(hr)) {
    LogD("CreateBuffer() failed for constant buffer..\n");
    return (hr);
...
void display(void) {
  ...
  ZeroMemory(&constantBufferDomainShader, sizeof(CBUFFER_DOMAIN_SHADER));
  ...
  gpID3D11DeviceContext->UpdateSubresource(
      ...
      0,                                         // index
      NULL,                                      // box
      &constantBufferDomainShader,               // source data
      0, 0  // row pitch and depth pitch of box
  );

  CBUFFER_HULL_SHADER constantBufferHullShader;
  ZeroMemory(&constantBufferHullShader, sizeof(CBUFFER_HULL_SHADER));
  constantBufferHullShader.HullConstantFunctionParams =
      XMVectorSet(1.0f, (FLOAT)guiNumberOfLineSegments, 0.0f, 1.0f);
  ...
  gpID3D11DeviceContext->UpdateSubresource(
      ...
      0,                                       // index
      NULL,                                    // box
      &constantBufferHullShader,               // source data
      0, 0  // row pitch and depth pitch of box
  );
  ...
  gpID3D11DeviceContext->UpdateSubresource(
      ...
      0,                                        // index
      NULL,                                     // box
      &constantBufferPixelShader,               // source data
      0, 0  // row pitch and depth pitch of box
  );
