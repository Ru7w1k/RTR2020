...
ID3D11Buffer *gpID3D11BufferVertexBufferPositionPyramid = NULL;
ID3D11Buffer *gpID3D11BufferVertexBufferColorPyramid = NULL;
ID3D11Buffer *gpID3D11BufferVertexBufferPositionCube = NULL;
ID3D11Buffer *gpID3D11BufferVertexBufferColorCube = NULL;
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance,
  ...
  // create window
  hwnd = CreateWindowEx(
      WS_EX_APPWINDOW, szAppName, TEXT("D3D11 | Pyramid and Cube color"),
...
HRESULT initialize(void) {
  ...
  // color data
  inputElementDesc[1].SemanticName = "COLOR";  // ~ vColor
  inputElementDesc[1].SemanticIndex = 0;       // multiple data (like mat4)
  inputElementDesc[1].Format = DXGI_FORMAT_R32G32B32_FLOAT;  //
  inputElementDesc[1].AlignedByteOffset = 0;                 // offset
  inputElementDesc[1].InputSlot = 1;  // ~ enum RMC_ATTRIBUTE_COLOR
  inputElementDesc[1].InputSlotClass =
      D3D11_INPUT_PER_VERTEX_DATA;               // per vertex
  inputElementDesc[1].InstanceDataStepRate = 0;  // useful if per instance data
  ...
  hr = gpID3D11Device->CreateBuffer(
      ...
      &gpID3D11BufferVertexBufferPositionPyramid  // vertex buffer !
  );
  if (FAILED(hr)) {
  ...
  gpID3D11DeviceContext->Map(
      gpID3D11BufferVertexBufferPositionPyramid,  // which buffer to map
  ...
  gpID3D11DeviceContext->Unmap(gpID3D11BufferVertexBufferPositionPyramid,
                               0 /* index */);
  ...
  hr = gpID3D11Device->CreateBuffer(
      &bufferDesc,  // buffer desc
      NULL,         // subresource data (used in static buffer)
      &gpID3D11BufferVertexBufferColorPyramid  // vertex buffer !
  );
  if (FAILED(hr)) {
    LogD("CreateBuffer() failed..\n");
    return (hr);
  ...
  ZeroMemory(&mappedSubresource, sizeof(D3D11_MAPPED_SUBRESOURCE));
  gpID3D11DeviceContext->Map(
      gpID3D11BufferVertexBufferColorPyramid,  // which buffer to map
      0,                                       // which index to use
      D3D11_MAP_WRITE_DISCARD,                 // how to map
      0,                                       // map flags (do not wait)
      &mappedSubresource                       // mapped pointer
  ...
  gpID3D11DeviceContext->Unmap(gpID3D11BufferVertexBufferColorPyramid,
                               0 /* index */);
  ...
  bufferDesc_VertexBuffer_Cube_Position.ByteWidth =
      sizeof(float) * ARRAYSIZE(verticesCube);
  ...
  hr =
      gpID3D11Device->CreateBuffer(&bufferDesc_VertexBuffer_Cube_Position, NULL,
                                   &gpID3D11BufferVertexBufferPositionCube);
  if (FAILED(hr)) {
    LogD("CreateBuffer() failed..\n");
    return (hr);
  }
  ...
  ZeroMemory(&mappedSubresource, sizeof(D3D11_MAPPED_SUBRESOURCE));
  gpID3D11DeviceContext->Map(gpID3D11BufferVertexBufferPositionCube, 0,
                             D3D11_MAP_WRITE_DISCARD, 0, &mappedSubresource);
  memcpy(mappedSubresource.pData, verticesCube, sizeof(verticesCube));
  gpID3D11DeviceContext->Unmap(gpID3D11BufferVertexBufferPositionCube, NULL);

  /////////////////////////////////////////////////////////////////////////////////////

  // create vertex buffer for rectangle colors
  D3D11_BUFFER_DESC bufferDesc_VertexBuffer_Cube_Color;
  ZeroMemory(&bufferDesc_VertexBuffer_Cube_Color, sizeof(D3D11_BUFFER_DESC));
  bufferDesc_VertexBuffer_Cube_Color.Usage = D3D11_USAGE_DYNAMIC;
  bufferDesc_VertexBuffer_Cube_Color.ByteWidth =
      sizeof(float) * ARRAYSIZE(verticesCube);
  bufferDesc_VertexBuffer_Cube_Color.BindFlags = D3D11_BIND_VERTEX_BUFFER;
  bufferDesc_VertexBuffer_Cube_Color.CPUAccessFlags = D3D10_CPU_ACCESS_WRITE;

  hr = gpID3D11Device->CreateBuffer(&bufferDesc_VertexBuffer_Cube_Color, NULL,
                                    &gpID3D11BufferVertexBufferColorCube);
  if (FAILED(hr)) {
    LogD("CreateBuffer() failed..\n");
    return (hr);
  }
  ...
  // copy vertices into above buffer
  ZeroMemory(&mappedSubresource, sizeof(D3D11_MAPPED_SUBRESOURCE));
  gpID3D11DeviceContext->Map(gpID3D11BufferVertexBufferColorCube, 0,
                             D3D11_MAP_WRITE_DISCARD, 0, &mappedSubresource);
  memcpy(mappedSubresource.pData, colorsCube, sizeof(colorsCube));
  gpID3D11DeviceContext->Unmap(gpID3D11BufferVertexBufferColorCube, NULL);
...
void display(void) {
  ...
  offset = 0;
  gpID3D11DeviceContext->IASetVertexBuffers(
      0, 1, &gpID3D11BufferVertexBufferPositionCube, &stride, &offset);

  // color
  stride = sizeof(float) * 3;
  offset = 0;
  gpID3D11DeviceContext->IASetVertexBuffers(
      1, 1, &gpID3D11BufferVertexBufferColorCube, &stride, &offset);
  ...
  XMMATRIX r1 = XMMatrixRotationX(XMConvertToRadians(
      -angleCube));  // +ve: right to left | -ve: left to right rotation
  ...
  constantBuffer.WorldViewProjectionMatrix = wvpMatrix;
  gpID3D11DeviceContext->UpdateSubresource(gpID3D11BufferConstantBuffer, 0,
...
void uninitialize(void) {
  ...
  if (gpID3D11BufferVertexBufferColorPyramid) {
    gpID3D11BufferVertexBufferColorPyramid->Release();
    gpID3D11BufferVertexBufferColorPyramid = NULL;
  }

  if (gpID3D11BufferVertexBufferPositionPyramid) {
    gpID3D11BufferVertexBufferPositionPyramid->Release();
    gpID3D11BufferVertexBufferPositionPyramid = NULL;
  }
