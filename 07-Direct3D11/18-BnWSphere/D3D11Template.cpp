...
ID3D11Buffer *gpID3D11BufferVertexBufferNormal = NULL;
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance,
  ...
  // create window
  hwnd = CreateWindowEx(
      WS_EX_APPWINDOW, szAppName, TEXT("D3D11 | BnW Sphere"),
...
HRESULT initialize(void) {
  ...
  // normal data
  inputElementDesc[1].SemanticName = "NORMAL";  // ~ vNormal
  ...
  bufferDesc.ByteWidth = sizeof(float) * gNumVertices * 3;
  ...
  memcpy(mappedSubresource.pData, sphere_vertices,
         sizeof(float) * gNumVertices * 3);
  ...
  hr = gpID3D11Device->CreateBuffer(
      &bufferDesc,  // buffer desc
      NULL,         // subresource data (used in static buffer)
      &gpID3D11BufferVertexBufferNormal  // vertex buffer !
  );
  if (FAILED(hr)) {
    LogD("CreateBuffer() failed..\n");
    return (hr);
  ...
  ZeroMemory(&mappedSubresource, sizeof(D3D11_MAPPED_SUBRESOURCE));
  gpID3D11DeviceContext->Map(
      gpID3D11BufferVertexBufferNormal,  // which buffer to map
      0,                                 // which index to use
      D3D11_MAP_WRITE_DISCARD,           // how to map
      0,                                 // map flags (do not wait)
      &mappedSubresource                 // mapped pointer
  );
  memcpy(mappedSubresource.pData, sphere_normals,
         sizeof(float) * gNumVertices * 3);
  gpID3D11DeviceContext->Unmap(gpID3D11BufferVertexBufferNormal, 0 /* index */);
  ...
  hr = gpID3D11Device->CreateBuffer(
      &bufferDesc,                // buffer desc
      NULL,                       // subresource data (used in static buffer)
      &gpID3D11BufferIndexBuffer  // index buffer !
  );
  if (FAILED(hr)) {
    LogD("CreateBuffer() failed..\n");
    return (hr);
  ...
  ZeroMemory(&mappedSubresource, sizeof(D3D11_MAPPED_SUBRESOURCE));
  gpID3D11DeviceContext->Map(gpID3D11BufferIndexBuffer,  // which buffer to map
                             0,                          // which index to use
                             D3D11_MAP_WRITE_DISCARD,    // how to map
                             0,                  // map flags (do not wait)
                             &mappedSubresource  // mapped pointer
  ...
  gpID3D11DeviceContext->Unmap(gpID3D11BufferIndexBuffer, 0 /* index */);
...
void display(void) {
  ...
  gpID3D11DeviceContext->IASetIndexBuffer(gpID3D11BufferIndexBuffer,
...
void update(void) {
  // code
  angleRotation += 1.0f;
  if (angleRotation >= 360.0f) angleRotation = 0.0f;
}
...
void uninitialize(void) {
  ...
  if (gpID3D11BufferVertexBufferNormal) {
    gpID3D11BufferVertexBufferNormal->Release();
    gpID3D11BufferVertexBufferNormal = NULL;
  }
