...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance,
  ...
  if (fopen_s(&gpFile, gszLogFileName, "w") != 0) {
    MessageBox(NULL, TEXT("Cannot Create log file!"), TEXT("Error"),
  ...
  wndclass.hIcon = LoadIcon(NULL, MAKEINTRESOURCE(RMC_ICON));
  wndclass.hCursor = LoadCursor(NULL, IDC_ARROW);
  wndclass.lpszClassName = szClassName;
  wndclass.lpszMenuName = NULL;
  wndclass.hIconSm = LoadIcon(NULL, MAKEINTRESOURCE(RMC_ICON));
  ...
  // create window
  hwnd = CreateWindow(
      szClassName, TEXT("D3D11 | Pyramid and Cube Texture"),
  ...
  while (bDone == false) {
    if (PeekMessage(&msg, NULL, 0, 0, PM_REMOVE)) {
      ...
      if (gbActiveWindow == true) {
        if (gbEscapeKeyIsPressed == true) bDone = true;
      }
    }
  }
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT iMsg, WPARAM wParam, LPARAM lParam) {
  ...
  switch (iMsg) {
    ...
    case WM_KEYDOWN:
      switch (wParam) {
        case VK_ESCAPE:
          if (gbEscapeKeyIsPressed == false) gbEscapeKeyIsPressed = true;
          break;
...
HRESULT initialize(void) {
  ...
  D3D_FEATURE_LEVEL d3dFeatureLevel_acquired =
      D3D_FEATURE_LEVEL_10_0;  // default, lowest
  ...
  // code
  numDriverTypes = sizeof(d3dDriverTypes) /
                   sizeof(d3dDriverTypes[0]);  // calculating size of array
  ...
  hr = gpID3D11Device->CreateInputLayout(
      inputElementsDesc, _ARRAYSIZE(inputElementsDesc),
      pID3DBlob_VertexShaderCode->GetBufferPointer(),
  ...
  bufferDesc_VertexBuffer_Pyramid_Position.ByteWidth =
      sizeof(float) * ARRAYSIZE(vertices_pyramid);
  ...
  if (FAILED(hr)) {
    fopen_s(&gpFile, gszLogFileName, "a+");
    fprintf_s(gpFile,
              "ID3D11Device::CreateBuffer() failed for Vertex Buffer.\n");
    fclose(gpFile);
    return (hr);
  } else {
    fopen_s(&gpFile, gszLogFileName, "a+");
  ...
  D3D11_BUFFER_DESC bufferDesc_VertexBuffer_Pyramid_Texture;
  ZeroMemory(&bufferDesc_VertexBuffer_Pyramid_Texture,
             sizeof(D3D11_BUFFER_DESC));
  bufferDesc_VertexBuffer_Pyramid_Texture.Usage = D3D11_USAGE_DYNAMIC;
  bufferDesc_VertexBuffer_Pyramid_Texture.ByteWidth =
      sizeof(float) * ARRAYSIZE(texture_pyramid);
  bufferDesc_VertexBuffer_Pyramid_Texture.BindFlags = D3D11_BIND_VERTEX_BUFFER;
  bufferDesc_VertexBuffer_Pyramid_Texture.CPUAccessFlags =
      D3D10_CPU_ACCESS_WRITE;

  hr = gpID3D11Device->CreateBuffer(
      &bufferDesc_VertexBuffer_Pyramid_Texture, NULL,
  ...
  gpID3D11DeviceContext->Unmap(gpID3D11Buffer_VertexBuffer_Pyramid_Texture,
                               NULL);
  ...
  bufferDesc_VertexBuffer_Cube_Position.ByteWidth =
      sizeof(float) * ARRAYSIZE(vertices_cube);
  ...
  hr =
      gpID3D11Device->CreateBuffer(&bufferDesc_VertexBuffer_Cube_Position, NULL,
  ...
  // create vertex buffer for rectangle textures
  D3D11_BUFFER_DESC bufferDesc_VertexBuffer_Cube_Texture;
  ZeroMemory(&bufferDesc_VertexBuffer_Cube_Texture, sizeof(D3D11_BUFFER_DESC));
  bufferDesc_VertexBuffer_Cube_Texture.Usage = D3D11_USAGE_DYNAMIC;
  bufferDesc_VertexBuffer_Cube_Texture.ByteWidth =
      sizeof(float) * ARRAYSIZE(vertices_cube);
  bufferDesc_VertexBuffer_Cube_Texture.BindFlags = D3D11_BIND_VERTEX_BUFFER;
  bufferDesc_VertexBuffer_Cube_Texture.CPUAccessFlags = D3D10_CPU_ACCESS_WRITE;

  hr = gpID3D11Device->CreateBuffer(&bufferDesc_VertexBuffer_Cube_Texture, NULL,
  ...
  hr = gpID3D11Device->CreateBuffer(&bufferDesc_ConstantBuffer, NULL,
  ...
  if (FAILED(hr)) {
    ...
    fprintf_s(
        gpFile,
        "ID3D11Device::CreateRasterizerState() failed for Constant Buffer.\n");
    fclose(gpFile);
    return (hr);
  } else {
    fopen_s(&gpFile, gszLogFileName, "a+");
    fprintf_s(gpFile,
              "ID3D11Device::CreateRasterizerState() succeeded for Constant "
              "Buffer.\n");
    fclose(gpFile);
  }
  ...
  hr =
      LoadD3DTexture(L"Stone.bmp", &gpID3D11ShaderResourceView_Pyramid_Texture);
  if (FAILED(hr)) {
    fopen_s(&gpFile, gszLogFileName, "a+");
  ...
  hr = gpID3D11Device->CreateSamplerState(
      &samplerDesc, &gpID3D11SamplerState_Pyramid_Texture);
  if (FAILED(hr)) {
    fopen_s(&gpFile, gszLogFileName, "a+");
    fprintf_s(
        gpFile,
        "ID3D11Device::CreateSamplerState() failed for Constant Buffer.\n");
    fclose(gpFile);
    return (hr);
  } else {
    fopen_s(&gpFile, gszLogFileName, "a+");
    fprintf_s(
        gpFile,
        "ID3D11Device::CreateSamplerState() succeeded for Constant Buffer.\n");
    fclose(gpFile);
  }
...
HRESULT LoadD3DTexture(const wchar_t *textureFileName,
                       ID3D11ShaderResourceView **ppID3D11ShaderResourceView) {
  // code
  HRESULT hr = S_OK;

  // create texture
  hr = DirectX::CreateWICTextureFromFile(gpID3D11Device, textureFileName, NULL,
...
void display(void) {
  ...
  gpID3D11DeviceContext->IASetVertexBuffers(
      0, 1, &gpID3D11Buffer_VertexBuffer_Pyramid_Position, &stride, &offset);

  // texcoords
  stride = sizeof(float) * 2;
  offset = 0;
  gpID3D11DeviceContext->IASetVertexBuffers(
      1, 1, &gpID3D11Buffer_VertexBuffer_Pyramid_Texture, &stride, &offset);

  // bind texture and sampler as pixel shader resource
  gpID3D11DeviceContext->PSSetShaderResources(
      0, 1, &gpID3D11ShaderResourceView_Pyramid_Texture);
  ...
  offset = 0;
  gpID3D11DeviceContext->IASetVertexBuffers(
      0, 1, &gpID3D11Buffer_VertexBuffer_Cube_Position, &stride, &offset);

  // texture
  stride = sizeof(float) * 2;
  offset = 0;
  gpID3D11DeviceContext->IASetVertexBuffers(
      1, 1, &gpID3D11Buffer_VertexBuffer_Cube_Texture, &stride, &offset);

  // bind texture and sampler as pixel shader resource
  gpID3D11DeviceContext->PSSetShaderResources(
      0, 1, &gpID3D11ShaderResourceView_Cube_Texture);
  gpID3D11DeviceContext->PSSetSamplers(0, 1,
                                       &gpID3D11SamplerState_Cube_Texture);
  ...
  // translations
  translationMatrix = XMMatrixTranslation(1.5f, 0.0f, 6.0f);
  XMMATRIX r1 = XMMatrixRotationX(XMConvertToRadians(
      -gAngleCube));  // +ve: right to left | -ve: left to right rotation
  XMMATRIX r2 = XMMatrixRotationY(XMConvertToRadians(
      -gAngleCube));  // +ve: right to left | -ve: left to right rotation
  XMMATRIX r3 = XMMatrixRotationZ(XMConvertToRadians(
      -gAngleCube));  // +ve: right to left | -ve: left to right rotation
...
void update(void) {
  ...
  if (gAnglePyramid >= 360.0f) gAnglePyramid = 0.0f;

  gAngleCube += 0.01f;

  if (gAngleCube >= 360.0f) gAngleCube = 0.0f;
}
