/*
 * Direct3D - Sphere (Phong Light per Vertex)
 */
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance,
  ...
  // create window
  hwnd = CreateWindowEx(
      WS_EX_APPWINDOW, szClassName, TEXT("D3D11 | Per pixel light on Sphere"),
...
HRESULT initialize(void) {
  ...
  // clang-format off
	const char *vertexShaderSourceCode =
		...
		"		output.light_direction = (float3)(light_position) - eyeCoordinate.xyz;" \
		"		output.viewer_vector = -eyeCoordinate.xyz;" \
		"	} " \
  ...
  // clang-format off
    ...
    "struct vertex_output" \
		...
		"{" \
		" float3 phong_ads_color = float3(1.0, 1.0, 1.0);" \

		" if (keyPressed == 1) {" \
		" float3 normTransformedNormal = normalize(input.transformed_normal);" \
		" float3 normLightDirection = normalize(input.light_direction);" \
		" float3 normViewerVector = normalize(input.viewer_vector);" \

		" float3 ambient = la * ka;" \
		" float3 diffuse = ld * kd * max(dot(normTransformedNormal, normLightDirection), 0.0);" \

		" float3 reflection_vector = reflect(-normLightDirection, normTransformedNormal);" \
		" float3 specular = ls * ks * pow(max(dot(reflection_vector, normViewerVector), 0.0), material_shininess); " \
		" phong_ads_color = ambient + diffuse + specular; }" \

		"	return(float4(phong_ads_color, 1.0));" \
		"}";
  // clang-format on
  ...
  if (FAILED(hr)) {
    ...
    fopen_s(&gpFile, gszLogFileName, "a+");
    fprintf_s(gpFile, "D3DCompile() succeeded for Pixel Shader: %s.\n",
              (char *)pID3DBlob_Error->GetBufferPointer());
    fclose(gpFile);
  }
  ...
  bufferDesc_VertexBuffer_Position.ByteWidth =
      sizeof(float) * ARRAYSIZE(sphere_vertices);
  ...
  hr = gpID3D11Device->CreateBuffer(&bufferDesc_VertexBuffer_Position, NULL,
  ...
  bufferDesc_VertexBuffer_Normal.ByteWidth =
      sizeof(float) * ARRAYSIZE(sphere_normals);
  ...
  hr = gpID3D11Device->CreateBuffer(&bufferDesc_VertexBuffer_Normal, NULL,
                                    &gpID3D11Buffer_VertexBuffer_Normal);
  if (FAILED(hr)) {
    fopen_s(&gpFile, gszLogFileName, "a+");
    fprintf_s(gpFile,
              "ID3D11Device::CreateBuffer() failed for Normal Buffer.\n");
    fclose(gpFile);
    return (hr);
  } else {
    fopen_s(&gpFile, gszLogFileName, "a+");
    fprintf_s(gpFile,
              "ID3D11Device::CreateBuffer() succeeded for Normal Buffer.\n");
    fclose(gpFile);
  }
  ...
  hr = gpID3D11Device->CreateBuffer(&bufferDesc_IndexBuffer, NULL,
                                    &gpID3D11Buffer_IndexBuffer);
  if (FAILED(hr)) {
    fopen_s(&gpFile, gszLogFileName, "a+");
    fprintf_s(gpFile,
              "ID3D11Device::CreateBuffer() failed for Index Buffer.\n");
    fclose(gpFile);
    return (hr);
  } else {
    fopen_s(&gpFile, gszLogFileName, "a+");
    fprintf_s(gpFile,
              "ID3D11Device::CreateBuffer() succeeded for Index Buffer.\n");
    fclose(gpFile);
  }
  ...
  gpID3D11DeviceContext->Map(gpID3D11Buffer_IndexBuffer, 0,
                             D3D11_MAP_WRITE_DISCARD, 0, &mappedSubresource);
  memcpy(mappedSubresource.pData, sphere_elements,
  ...
  gpID3D11DeviceContext->VSSetConstantBuffers(0, 1,
                                              &gpID3D11Buffer_ConstantBuffer);
  gpID3D11DeviceContext->PSSetConstantBuffers(0, 1,
                                              &gpID3D11Buffer_ConstantBuffer);
...
void display(void) {
  ...
  gpID3D11DeviceContext->IASetVertexBuffers(
      0, 1, &gpID3D11Buffer_VertexBuffer_Position, &stride, &offset);

  // normals
  stride = sizeof(float) * 3;
  offset = 0;
  gpID3D11DeviceContext->IASetVertexBuffers(
      1, 1, &gpID3D11Buffer_VertexBuffer_Normal, &stride, &offset);
  ...
  if (gbLight == true) {
    ...
    constantBuffer.La = XMVectorSet(lightAmbient[0], lightAmbient[1],
                                    lightAmbient[2], lightAmbient[3]);
    constantBuffer.Ld = XMVectorSet(lightDiffuse[0], lightDiffuse[1],
                                    lightDiffuse[2], lightDiffuse[3]);
    constantBuffer.Ls = XMVectorSet(lightSpecular[0], lightSpecular[1],
                                    lightSpecular[2], lightSpecular[3]);
    constantBuffer.Light_Position = XMVectorSet(
        lightPosition[0], lightPosition[1], lightPosition[2], lightPosition[3]);

    constantBuffer.Ka = XMVectorSet(materialAmbient[0], materialAmbient[1],
                                    materialAmbient[2], materialAmbient[3]);
    constantBuffer.Kd = XMVectorSet(materialDiffuse[0], materialDiffuse[1],
                                    materialDiffuse[2], materialDiffuse[3]);
    constantBuffer.Ks = XMVectorSet(materialSpecular[0], materialSpecular[1],
