/*
 * Direct3D - Sphere (Phong Light per Pixel)
 */
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT iMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CHAR:
		...
		case 'x':
			bXLight = !bXLight;
			bYLight = bZLight = false;
			break;

		case 'Y':
		case 'y':
			bYLight = !bYLight;
			bXLight = bZLight = false;
			break;

		case 'Z':
		case 'z':
			bZLight = !bZLight;
			bXLight = bYLight = false;
			break;
...
void display(void)
{
	...
	constantBuffer.ProjectionMatrix = XMMatrixPerspectiveFovLH(
		XMConvertToRadians(45.0f),
		((float)gWidth / 6.0f) / ((float)gHeight / 4.0f),
		0.1f,
		100.0f);
	if (gbLight == true)
	{
		...
		if (bXLight)
			constantBuffer.Light_Position = XMVectorSet(0.0f, 100.0f*cos(-angle), 100.0f*sin(-angle), 1.0f);

		if (bYLight)
			constantBuffer.Light_Position = XMVectorSet(100.0f*cos(-angle), 0.0f, 100.0f*sin(-angle), 1.0f);

		if (bZLight)
			constantBuffer.Light_Position = XMVectorSet(100.0f*cos(-angle), 100.0f*sin(-angle), 0.0f, 1.0f);
	}
	else
	{
	...
	// set viewport for each spehere
	// update cbuffer with respective materials
	D3D11_VIEWPORT d3dViewPort;
	d3dViewPort.MinDepth = 0.0f;
	d3dViewPort.MaxDepth = 1.0f;
	d3dViewPort.Width = (FLOAT)gWidth/6.0f;
	d3dViewPort.Height = (FLOAT)gHeight/4.0f;
	...
	for (int i = 0; i < 6; i++)
	{
		...
		{
			// viewport
			d3dViewPort.TopLeftX = i * (FLOAT)gWidth / 6.0f;
			d3dViewPort.TopLeftY = j * (FLOAT)gHeight / 4.0f;
			gpID3D11DeviceContext->RSSetViewports(1, &d3dViewPort);

			// material
			constantBuffer.Ka = XMVectorSet(Materials[i][3 - j].MaterialAmbient[0], Materials[i][3 - j].MaterialAmbient[1], Materials[i][3 - j].MaterialAmbient[2], Materials[i][3 - j].MaterialAmbient[3]);
			constantBuffer.Kd = XMVectorSet(Materials[i][3 - j].MaterialDiffuse[0], Materials[i][3 - j].MaterialDiffuse[1], Materials[i][3 - j].MaterialDiffuse[2], Materials[i][3 - j].MaterialDiffuse[3]);
			constantBuffer.Ks = XMVectorSet(Materials[i][3 - j].MaterialSpecular[0], Materials[i][3 - j].MaterialSpecular[1], Materials[i][3 - j].MaterialSpecular[2], Materials[i][3 - j].MaterialSpecular[3]);
			constantBuffer.Material_Shininess = Materials[i][3 - j].MaterialShininess[0];

			// send updated cbuffer
