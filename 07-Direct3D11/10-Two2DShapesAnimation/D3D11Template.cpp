...
ID3D11Buffer *gpID3D11BufferVertexBufferPositionTriangle = NULL;
ID3D11Buffer *gpID3D11BufferVertexBufferColorTriangle = NULL;

ID3D11Buffer *gpID3D11BufferVertexBufferPositionQuad = NULL;
ID3D11Buffer *gpID3D11BufferVertexBufferColorQuad = NULL;
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance,
  ...
  // create window
  hwnd = CreateWindowEx(
      WS_EX_APPWINDOW, szAppName, TEXT("D3D11 | Two 2D Shapes Animation"),
...
HRESULT initialize(void) {
  ...
  hr = gpID3D11Device->CreateBuffer(
      ...
      &gpID3D11BufferVertexBufferPositionTriangle  // vertex buffer !
  );
  if (FAILED(hr)) {
  ...
  gpID3D11DeviceContext->Map(
      gpID3D11BufferVertexBufferPositionTriangle,  // which buffer to map
  ...
  gpID3D11DeviceContext->Unmap(gpID3D11BufferVertexBufferPositionTriangle,
                               0 /* index */);
  ...
  hr = gpID3D11Device->CreateBuffer(
      &bufferDesc,  // buffer desc
      NULL,         // subresource data (used in static buffer)
      &gpID3D11BufferVertexBufferColorTriangle  // vertex buffer !
  );
  if (FAILED(hr)) {
    LogD("CreateBuffer() failed..\n");
    return (hr);
  ...
  ZeroMemory(&mappedSubresource, sizeof(D3D11_MAPPED_SUBRESOURCE));
  gpID3D11DeviceContext->Map(
      gpID3D11BufferVertexBufferColorTriangle,  // which buffer to map
      0,                                        // which index to use
      D3D11_MAP_WRITE_DISCARD,                  // how to map
      0,                                        // map flags (do not wait)
      &mappedSubresource                        // mapped pointer
  ...
  gpID3D11DeviceContext->Unmap(gpID3D11BufferVertexBufferColorTriangle,
                               0 /* index */);
  ...
  // create vertex buffer for position
  ZeroMemory(&bufferDesc, sizeof(D3D11_BUFFER_DESC));
  bufferDesc.ByteWidth = sizeof(float) * _ARRAYSIZE(verticesQuad);
  bufferDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;
  bufferDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
  bufferDesc.Usage = D3D11_USAGE_DYNAMIC;
  ...
  hr = gpID3D11Device->CreateBuffer(
      &bufferDesc,  // buffer desc
      NULL,         // subresource data (used in static buffer)
      &gpID3D11BufferVertexBufferPositionQuad  // vertex buffer !
  );
  if (FAILED(hr)) {
    LogD("CreateBuffer() failed..\n");
    return (hr);
  ...
  ZeroMemory(&mappedSubresource, sizeof(D3D11_MAPPED_SUBRESOURCE));
  gpID3D11DeviceContext->Map(
      gpID3D11BufferVertexBufferPositionQuad,  // which buffer to map
      0,                                       // which index to use
      D3D11_MAP_WRITE_DISCARD,                 // how to map
      0,                                       // map flags (do not wait)
      &mappedSubresource                       // mapped pointer
  ...
  gpID3D11DeviceContext->Unmap(gpID3D11BufferVertexBufferPositionQuad,
                               0 /* index */);

  // create vertex buffer for color
  ZeroMemory(&bufferDesc, sizeof(D3D11_BUFFER_DESC));
  bufferDesc.ByteWidth = sizeof(float) * _ARRAYSIZE(colorsQuad);
  bufferDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;
  bufferDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
  bufferDesc.Usage = D3D11_USAGE_DYNAMIC;
  ...
  hr = gpID3D11Device->CreateBuffer(
      &bufferDesc,  // buffer desc
      NULL,         // subresource data (used in static buffer)
      &gpID3D11BufferVertexBufferColorQuad  // vertex buffer !
  );
  if (FAILED(hr)) {
    LogD("CreateBuffer() failed..\n");
    return (hr);
  ...
  ZeroMemory(&mappedSubresource, sizeof(D3D11_MAPPED_SUBRESOURCE));
  gpID3D11DeviceContext->Map(
      gpID3D11BufferVertexBufferColorQuad,  // which buffer to map
      0,                                    // which index to use
      D3D11_MAP_WRITE_DISCARD,              // how to map
      0,                                    // map flags (do not wait)
      &mappedSubresource                    // mapped pointer
  );
  memcpy(mappedSubresource.pData, colorsQuad, sizeof(colorsQuad));
  gpID3D11DeviceContext->Unmap(gpID3D11BufferVertexBufferColorQuad,
                               0 /* index */);
...
void display(void) {
  ...
  translationMatrix = XMMatrixTranslation(1.5f, 0.0f, 6.0f);
  rotationMatrix = XMMatrixRotationX(XMConvertToRadians(-angleQuad));
...
void update(void) {
  ...
  if (angleTriangle >= 360.0f) angleTriangle = 0.0f;
   ...
   angleQuad += 1.0f;
  if (angleQuad >= 360.0f) angleQuad = 0.0f;
}
...
void uninitialize(void) {
  ...
  if (gpID3D11BufferVertexBufferColorTriangle) {
    gpID3D11BufferVertexBufferColorTriangle->Release();
    gpID3D11BufferVertexBufferColorTriangle = NULL;
  }

  if (gpID3D11BufferVertexBufferPositionTriangle) {
    gpID3D11BufferVertexBufferPositionTriangle->Release();
    gpID3D11BufferVertexBufferPositionTriangle = NULL;
  }
