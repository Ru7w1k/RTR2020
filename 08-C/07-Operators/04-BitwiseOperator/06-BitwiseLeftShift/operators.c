...
int main(void)
{
	...
	printf("\n\n");
	printf("By how many bits do you want to shift A = %d to left? : ", rmc_a);
	scanf("%d", &rmc_bits);

	printf("\n\n\n\n");
	rmc_result = rmc_a << rmc_bits;
	printf("Bitwise RIGHT-SHIFTing A = %d (decimal) by %d bits\nGives the result %d (decimal).\n\n", rmc_a, rmc_bits, rmc_result);
